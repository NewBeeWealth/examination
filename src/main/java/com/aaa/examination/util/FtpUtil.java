...
public class FtpUtil {
    ...
    static{
        InputStream resourceAsStream = FtpUtil.class.getResourceAsStream("/application.properties");
        Properties property = new Properties();
        try {
            property.load(resourceAsStream);
            System.out.println(property.get("remoteIp")+"..remoteIp..");
            map=property;
            System.out.println(map.get("remoteIp")+"..remoteIp2..");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    private  static String remoteIp=map.get("remoteIp")+"";

    private static  int uploadPort=Integer.valueOf(map.get("uploadPort")+"");

    private  static String ftpUserName=map.get("ftpUserName")+"";;

    private static String ftpPassWord=map.get("ftpPassWord")+"";;

    private static  String remotePath=map.get("remotePath")+"";

    private static  String uploadPath=map.get("upload.path")+"";

    private static  String localPath=map.get("localPath")+"";
    ...
    public static String  upLoad(MultipartFile multipartFile){
        ...
        try {
            ...
            //连接ftp服务器
            ftp.connect(remoteIp,uploadPort);
            //登录
